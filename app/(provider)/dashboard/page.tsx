...
export default function ProviderDashboard() {
  ...
  async function updateStatus(bookingId: string, newStatus: string) {
    ...
    if (booking) {
      await supabase.from("notifications").insert({
        ...
        message: `Your service booking status is now: ${newStatus.replace("_", " ")}`,
        type: "booking",
      });
    }
    fetchProviderAndBookings();
  ...
  async function toggleAvailability() {
    await supabase.from("providers").update({ available: !provider.available }).eq("id", provider.id);
    setProvider({ ...provider, available: !provider.available });
  }
    ...
    return (
      <div>
        ...
        <div className="max-w-md mx-auto px-4 py-16 text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">Please register as a provider first.</p>
          <a href="/register" className="mt-4 inline-block bg-orange-500 text-white px-6 py-3 rounded-xl font-semibold">Register</a>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white px-4 py-6">
        <div className="max-w-2xl mx-auto flex justify-between items-center">
          ...
          <button onClick={toggleAvailability}
            className={`px-4 py-2 rounded-full text-sm font-bold transition ${provider?.available ? "bg-green-400 text-white" : "bg-gray-300 text-gray-700"}`}>
      ...
      <div className="max-w-2xl mx-auto px-4 mt-4">
        ...
        <div className="flex bg-gray-100 rounded-xl p-1 mb-4">
          <button onClick={() => setTab("active")}
            className={`flex-1 py-2 rounded-lg text-sm font-semibold transition ${tab === "active" ? "bg-white text-orange-500 shadow-sm" : "text-gray-500"}`}>
            Active ({active.length})
          </button>
          <button onClick={() => setTab("completed")}
            className={`flex-1 py-2 rounded-lg text-sm font-semibold transition ${tab === "completed" ? "bg-white text-orange-500 shadow-sm" : "text-gray-500"}`}>
            Completed ({completed.length})
          </button>
        </div>

        {loading && <div className="space-y-3">{[1,2].map(i => <div key={i} className="bg-white rounded-2xl h-40 animate-pulse" />)}</div>}
              ...
              <div key={b.id} className="bg-white rounded-2xl shadow-sm p-5">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">👤 {b.profiles?.name}</p>
                ...
                <div className="bg-gray-50 rounded-xl p-3 mb-3 space-y-1 text-sm">
                  ...
                  {b.amount && <p><span className="text-gray-400">Budget:</span> <strong className="text-orange-500">₹{b.amount}</strong></p>}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  {nextStatus && b.status !== "pending" && (
                    <button onClick={() => updateStatus(b.id, nextStatus)}
